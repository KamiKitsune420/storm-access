...
void HookFunctions::InitializeHooks()
{
	...
	// ACCESSIBILITY BUILD: the content-mod hooks below patch battle-time game
	// code (cpk load, in-battle special conditions, partner spawning) that this
	// screen-reader mod does not use. They were crashing on battle entry, so we
	// disable them - only the message-read hooks above are needed.
#if 0
	// Hook Cpk Load
	if (gameVersion == "1.09")
	{
