...
// The message hooks patch the game's code in-memory (unhook -> call original ->
// re-hook) on every conversion. During battle/cutscene transitions this fires
// from multiple threads at once; two threads patching the same bytes crashes.
// Serialize the whole critical section. Recursive because one message hook can
// legitimately re-enter the other on the same thread.
static std::recursive_mutex g_msgHookMtx;
...
{
	std::lock_guard<std::recursive_mutex> _hookLock(g_msgHookMtx);
	...
	if (ccGeneralGameFunctions::ViewMessageConversions == 0 && strlen((char*)MessageToDecode) > 0 && *(char*)MessageToDecode != '+')
	{
		...
		Accessibility::OnMessageDecoded((char*)MessageToDecode, result);
